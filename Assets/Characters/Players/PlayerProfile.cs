...
public class Sniper : CharacterProfile
{
    public Sniper ( float initial_health, float speed_multiplier ) :
        base ( CharacterType.SNIPER, initial_health, speed_multiplier )
    {
        //add green baret characteristics here
        // 1. collectables along with defaults

        // 2. abilities along with defaults
        abilities.Add ( AbilityType.SNIPING );
    }

    public override void PickUp ( CollectableType ctype )
    {
        Debug.Log ( "sniper requested to pickup " + ctype );
        //first check  to see if character can pickup ctype
    }
}//sniper


public class Sapper : CharacterProfile
{
    public Sapper ( float initial_health, float speed_multiplier ) :
        base ( CharacterType.SAPPER, initial_health, speed_multiplier )
    {
        //add green baret characteristics here
        // 1. collectables along with defaults

        // 2. abilities along with defaults
        abilities.Add ( AbilityType.TRAP );
    }

    public override void PickUp ( CollectableType ctype )
    {
        Debug.Log ( "sapper requested to pickup " + ctype );
        //first check  to see if character can pickup ctype
    }
}//sniper